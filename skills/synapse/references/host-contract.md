...
- `useDataSync(cb)` → on **0.19.0+**, re-fetch when **your own server** announces a write, by sending `notifications/resources/list_changed`; the host relays it to that server's views, and the callback receives the notification's params. Older SDKs listen for the `synapse/data-changed` extension instead — `portability.md`'s version note.
