...
- `useDataSync` (0.19.0+) fires on your server's `notifications/resources/list_changed`, so in preview it fires exactly when your server announces one. Announce only after a **write** commits: a read that announces makes every view refresh forever. On an older SDK it listens for `synapse/data-changed` instead — `portability.md`'s version note.
...
## H. Bridge & hooks
The bridge is the MCP **ext-apps spec (`2026-01-26`)** plus NimbleBrain `synapse/*` extensions (`action`, `request-file`, `download-file`, `persist-state`/`state-loaded`, `fontFaces`, `keydown`). **They do not all degrade the same way** — `request-file` *throws* off NimbleBrain while `action` returns silently and `download-file` is sent unguarded and dropped, so "degrades gracefully" is not something to code against. Feature-detect with `useSynapse().isNimbleBrainHost`, minding that it is unset at first render and carries no subscription — `portability.md` ranks the ways to handle that; per-hook wire method and exact behavior: `portability.md`. Live refresh is **not** in that list from 0.19.0: `useDataSync` rides the spec's own `notifications/resources/list_changed`, so any MCP server gets it by announcing its own writes. Older SDKs ride the extension — `portability.md`'s version note.

Hooks on the `createSynapse`/`SynapseProvider` path: `useSynapse, useCallTool, useCallToolAsTask, useDataSync, useTheme, useHostContext, useVisibleState, useFileUpload, useChat, useAction, useAgentAction, useStore`. The `connect()`/`AppProvider` path exports its own five: `useApp, useToolResult, useToolInput, useConnectTheme, useResize`.

## J. Package hard rules
Per the SDK's own contributor docs: never hand-type ext-apps method strings or message param shapes (import the constants/types from `@modelcontextprotocol/ext-apps`); never `as any` a content block; use `connect()` for standalone widgets vs `createSynapse()`/`SynapseProvider` for host apps. As of **0.13.0** there is a third entry point, `connectUI()` — a push-first cross-host client that feature-detects the browsing context, picks a per-host adapter, and exposes `capabilities()` so a widget can branch on what the host offers. It is widget-shaped, not a substitute for `SynapseProvider` in a full app (`portability.md`).
