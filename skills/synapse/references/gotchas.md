...
- **The CSS is applied once, from the handshake.** A host that swaps its typeface mid-session does not move a running app.
- If you *are* the host: send the `@font-face` rules in the `ui/initialize` answer, give every `--font-*` token value a **web-safe tail** (`"'Your Sans', system-ui, sans-serif"`, never `"'Your Sans'"`), and check the frame's CSP — a `srcdoc` iframe without `allow-same-origin` has an **opaque origin**, so `font-src 'self'` matches nothing and only a `data:` URI works unconditionally.
