...
- `useDataSync` fires on your server's `notifications/resources/list_changed`, so in preview it fires exactly when your server announces one. Announce only after a **write** commits: a read that announces makes every view refresh forever.
